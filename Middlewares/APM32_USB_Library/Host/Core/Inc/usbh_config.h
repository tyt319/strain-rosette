...
/*!< [31:16] APM32 USB Host Library main version V1.2.2*/
#define __APM32_USB_HOST_VERSION_MAIN   (0x01) /*!< [31:24] main version */
#define __APM32_USB_HOST_VERSION_SUB1   (0x02) /*!< [23:16] sub1 version */
#define __APM32_USB_HOST_VERSION_SUB2   (0x02) /*!< [15:8]  sub2 version */
...
#define USBH_DEVICE_RESET_TIMEOUT           1000
#define USBH_DEVICE_RESET_MAX_CNT           3
#define USBH_DEVICE_DEFAULT_ADDRESS         0
#define USBH_DEVICE_CONFIGURED_ADDRESS      1
#define USBH_MAX_ERROR                      2
...
typedef enum
{
    USBH_ENUM_IDLE = 0,                     //!< enum idle
    USBH_ENUM_GET_DEV_DESC,                 //!< Get device descriptor
    USBH_ENUM_SET_ADDRESS,                  //!< Set address
    ...
    USBH_ENUM_GET_SERIAL_NUM_STRING_DESC,   //!< Get serial number string
} USBH_ENUM_STA_T;
...
/**
 * @brief   USB Host setup request data
 */
typedef struct
{
    union
    {
        uint32_t data[2]; //!< For DMA
...
typedef struct
{
    ...
    uint8_t         bInterfaceProtocol;  //!< Protocol Code
    uint8_t         iInterface;          //!< Index of String Descriptor of this interface
    USBH_EP_DESC_T  endpoint[ENDPOINT_DESC_MAX_NUM];
} USBH_INTERFACE_T;
...
typedef struct
{
    ...
    __IO uint8_t    reEnumerateStatus;
    uint8_t         portEnabledStatus;
    uint8_t         curItf;
...
typedef struct
{
    ...
    uint16_t                length;
    uint16_t                timer;
    USBH_SETUP_REQ_DATA_T   steupReq;
    USBH_CTRL_STATE_T       state;
    uint8_t                 errCnt;
} USBH_CTRL_T;
...
{
    __IO USBH_HOST_STA_T    hostState;                                      /*!< USB Host State Machine */
    USBH_ENUM_STA_T         hostEnumState;                                  /*!< USB Host Enumeration State */
    USBH_XFER_STA_T         xferState;                                      /*!< USB Transfer State */
    USBH_CTRL_T             ctrlInfo;                                       /*!< Control Transfer Information */
    USBH_DEV_INFO_T         devInfo;                                        /*!< USB Device Information */
    USBH_CLASS_T            *hostClass[USBH_SUP_CLASS_MAX_NUM];             /*!< USB Host Class */
    USBH_CLASS_T            *activeClass;                                   /*!< Active USB Host Class */
    uint32_t                classNum;                                       /*!< Number of USB Host Class */
    uint32_t                xferChannel[USBH_CHANNEL_MAX_NUM];              /*!< Transfer Channel */
    __IO uint32_t           timer;                                          /*!< Timer */
    uint32_t                timeout;                                        /*!< Timeout */
    USBH_SPEED_T            hostSpeed;                                      /*!< USB Host Speed */
    void                    *dataPoint;                                     /*!< Data Point */
    void (*userCallback)(struct _USBH_INFO_T* usbInfo, uint8_t userStatus); /*!< User Callback */
} USBH_INFO_T;
