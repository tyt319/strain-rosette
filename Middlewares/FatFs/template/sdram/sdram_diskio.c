...
{
    memcpy(buff, (uint32_t *)(SDRAM_DEVICE_ADDR + (sector * SDRAM_BLOCK_SIZE)), (SDRAM_BLOCK_SIZE * count) / 4U);
...
{
    memcpy((uint32_t *)(SDRAM_DEVICE_ADDR + (sector * SDRAM_BLOCK_SIZE)), buff, (SDRAM_BLOCK_SIZE * count) / 4U);
...
DRESULT SDRAM_DiskIoctl(BYTE lun, BYTE cmd, void *buff)
{
    ...
    switch (cmd)
    {
        ...
        case GET_SECTOR_COUNT:
            *(DWORD*)buff = SDRAM_DEVICE_SIZE / SDRAM_BLOCK_SIZE;
            res = RES_OK;
            break;

        case GET_SECTOR_SIZE:
            *(DWORD*)buff = SDRAM_BLOCK_SIZE;
            res = RES_OK;
            break;
